...
*/export const getResourceAuditTrail = async (resource: string, resourceId: string) => {return await prisma.auditLog.findMany({ where: { resource, resourceId }, include: { user: { select: { id: true, name: true, email: true } }, admin: {
          select: {
            id: true,
            name: true,
            email: true,
