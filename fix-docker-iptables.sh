...
# Specifically addresses "SKIP DNAT rule" and "iptables failed" errors
...
# Function to configure iptables backend
configure_iptables_backend() {
    ...
    # Try to switch to iptables-legacy if available
    if command -v iptables-legacy > /dev/null 2>&1; then
        echo "🔄 Switching to iptables-legacy..."
        sudo update-alternatives --set iptables /usr/sbin/iptables-legacy 2>/dev/null || echo "Could not switch to iptables-legacy"
    fi
    
    # Try to switch to iptables-nft if available
    if command -v iptables-nft > /dev/null 2>&1; then
        echo "🔄 Switching to iptables-nft..."
        sudo update-alternatives --set iptables /usr/sbin/iptables-nft 2>/dev/null || echo "Could not switch to iptables-nft"
    fi
}
...
main() {
    ...
    # Step 3: Configure iptables backend
    configure_iptables_backend
