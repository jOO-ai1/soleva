...
export const getMockProducts = (): MockProduct[] => [
{
  id: '1',
  name: {
    en: 'Classic Oxford Shoes',
    ar: 'حذاء أكسفورد كلاسيكي'
  },
  description: {
...
export const searchMockProducts = (query: string, lang: 'en' | 'ar' = 'en'): MockProduct[] => {
  const products = getMockProducts();
  const searchTerm = query.toLowerCase();

  return products.filter((product) =>
  product.name[lang].toLowerCase().includes(searchTerm) ||
  product.description[lang].toLowerCase().includes(searchTerm) ||
  product.category.name[lang].toLowerCase().includes(searchTerm)
  );
};

export const getProductsByCategory = (categorySlug: string): MockProduct[] => {
  const products = getMockProducts();
  return products.filter((product) => product.category.slug === categorySlug);
};

export const getProductsByCollection = (collectionSlug: string): MockProduct[] => {
  const products = getMockProducts();
