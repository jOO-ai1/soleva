...
export const useDeviceDetection = () => {
  ...
  const measurePerformanceMetrics = (): Promise<PerformanceMetrics> => {
    return new Promise((resolve) => {
      ...
      setTimeout(() => {fcpObserver.disconnect();lcpObserver.disconnect();clsObserver.disconnect();inpObserver.disconnect();metrics.loadTime = performance.now() - startTime;resolve(metrics as PerformanceMetrics);
        }, 5000);
    });
  };
